<template>
    ...
    <div v-else class="quick-view">
        <div class="quick-view-inner">
            <h2 class="d-flex justify-content-between">請登入以繼續使用
                <slot></slot>
            </h2>
            <div class="link-to-login" style="text-align: center; margin-top: 120px;">
                <router-link class="btn" to="/login" style="padding: 28px; font-size: 24px">登入
                </router-link>
            </div>
        </div>
    </div>
    ...
    methods: {
        ...
        async addToCart() {
            ...
            if (existItem.data.length == 1) {
                ...
                this.$refs.alert.showAlert('success', 'Thank you!', '成功加入購物車!')
