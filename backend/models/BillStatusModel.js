...
// get all Bills Status by date
export const getBillsByToday = (date, result) => {
    db.query("SELECT * FROM billstatus WHERE DATE(bill_when) = ?", date, (err, results) => {
        if (err) {
            console.log(err);
            result(err, null);
        } else {
...
export const getAll = (result) => {
    db.query("SELECT billstatus.*, seat.seat_status , seat.seat_id FROM billstatus LEFT JOIN seat ON billstatus.seat_id = seat.seat_id ", (err,results)=> {
        if (err){
            console.log(err);
            result(err,null);
        }
